...
export type SiteSettings = {
  ...
  notifyRsvp: boolean;
  fixedNav: NavEntry[];
  /** Per-site prefix for support ticket numbers (e.g. "SA" → SA-0007). */
 ...
 *      — so a freshly-cloned site already has distinct per-page titles
 *      for SEO without anyone editing the SEO panel first. Once the
 *      agent edits per-page meta in /admin/seo, those values take over.
 */
...
export function buildFallbackPageMeta(
  ...
  const area = s.serviceArea || "Northern Virginia & Maryland";
  ...
  switch (pageKey) {
    ...
    case "path-to-ownership":
      return {
        title: `Path to Ownership | ${name} — ${area}`,
        description: `A guided 12-to-24-month plan from renting to closing. No pressure, no cost to start. ${name} walks you through every step.`,
      };
    case "communities":
      return {
